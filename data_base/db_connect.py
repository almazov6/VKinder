...
from data_base.db_configs import DB_NAME, DB_PASSWORD, DB_LOGIN

conn = psycopg2.connect(database=DB_NAME, user=DB_LOGIN,
...
def get_user(vk_id_user):
    """Принимает в качестве аргумента vk_id пользователя приложения и выдает из базы информацию о нем для дальнейшего использования."""
    with (conn.cursor() as cur):
        cur.execute(f"""
...
def get_shown(vk_id):
    """Выводит список избранных со ссылкой на аккаунт и самой популярной фотографией."""
    with conn.cursor() as cur:
        cur.execute(f"""
...
def del_shown_user(shown_vk_id):
    ...
    with conn.cursor() as cur:
        cur.execute(f"""
                    DELETE FROM photos WHERE shown_id = (SELECT id FROM shown WHERE vk_id_searched = '{shown_vk_id}');
                    DELETE FROM shown WHERE vk_id_searched = '{shown_vk_id}';
                    """)
        conn.commit()
...
def add_photos(user_id):
    """Добавляет фотографии избранного варианта в базу данных."""
    photo = Vkapi().photos_get(user_id)[0][1]
    with conn.cursor() as cur:
        cur.execute(f"""
                INSERT INTO photos(id, shown_id, photo_url)
                VALUES (DEFAULT, (SELECT id FROM shown WHERE vk_id_searched = '{user_id}'), '{photo}')
                """)
        conn.commit()
...
def get_photo(user_id):
    ...
    with conn.cursor() as cur:
        cur.execute(f"""
            ...
            WHERE shown_id = (SELECT id FROM shown WHERE vk_id_searched = '{user_id}');
            """)
        result = cur.fetchone()
        return result[0]
